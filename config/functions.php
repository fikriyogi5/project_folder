...
function generateBreadcrumb()
{
    ...
    $breadcrumb .= '<li><a href="/">Home</a></li>'; // Link ke halaman utama

    $currentPath = '';
    foreach ($crumbs as $crumb) {
        if ($crumb !== '') {
            $currentPath .= '/' . $crumb;
            $breadcrumb .= '<li><a href="' . $currentPath . '">' . ucfirst($crumb) . '</a></li>';
        }
    }
...
function isBirthday($birthDate)
{
    ...
    // Bandingkan tanggal lahir dengan tanggal hari ini (tanpa memperhitungkan tahun)
    // return $birthdateObj->format('md') === $today->format('md');

    // Contoh penggunaan
    // $birthdate = '1990-11-21'; // Ganti dengan tanggal lahir yang sesuai
