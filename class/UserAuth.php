...
class UserAuth {
    ...
    public function autoLog($user_id, $activity_type, $activity_detail) {
        try {
            ...
            $stmt = $this->pdo->prepare("INSERT INTO user_activities (user_id, activity_type, activity_details) VALUES (:user_id, :activity_type, :activity_details)");
    ...
    //         $stmt = $this->pdo->prepare("INSERT INTO riwayat_pencarian (user_id, kata_kunci) VALUES (:user_id, :kata_kunci, :activity_details)");
    //         $stmt->bindParam(':user_id', $user_id, PDO::PARAM_INT);
    ...
    public function register($username, $email, $password) {
        try {
            ...
            $stmt = $this->pdo->prepare("INSERT INTO users (username, email, password, drive_size, verification_token, verification_created_at) VALUES (:username, :email, :password, :drive_size, :verification_token, :verification_created_at)");
    ...
    public function verifyEmail($verificationToken) {
        try {
            $stmt = $this->pdo->prepare("SELECT * FROM users WHERE verification_token = :verification_token");
            $stmt->bindParam(':verification_token', $verificationToken, \PDO::PARAM_STR);
            $stmt->execute();
    ...
    public function updateVerificationToken($email, $newVerificationToken, $newVerificationCreatedAt) {
        try {
            $stmt = $this->pdo->prepare("UPDATE users SET verification_token = :newVerificationToken, verification_created_at = :newVerificationCreatedAt WHERE email = :email");
    ...
    private function removeVerificationToken($data, $userId) {
        try {
            $updateStmt = $this->pdo->prepare("UPDATE users SET verification_token = NULL WHERE $data = :user_id");
    ...
    private function markUserAsVerified($userId) {
        $updateStmt = $this->pdo->prepare("UPDATE users SET verified = 1 WHERE id = :user_id");
        $updateStmt->bindParam(':user_id', $userId, \PDO::PARAM_INT);
        $updateStmt->execute();
    }
    ...
    public function generateAndSendOTP($email) {
        ...
        $stmt = $this->pdo->prepare("UPDATE users SET otp = :otp WHERE email = :email");
    ...
    public function verifyOTP($username, $enteredOTP) {
        try {
            // Prepare SQL statement to retrieve user data by username
            $stmt = $this->pdo->prepare("SELECT otp, user_role FROM users WHERE username = :username");
            $stmt->bindParam(':username', $username, PDO::PARAM_STR);
            $stmt->execute();
    ...
    public function checkData($data, $isi) {
        try {
            ...
            $stmt = $this->pdo->prepare("SELECT * FROM users WHERE $data = :isi");
    ...
    try {
        ...
        $stmt = $this->pdo->prepare("UPDATE users SET verification_token = :verification_token WHERE email = :email");
        $stmt->bindParam(':verification_token', $verificationToken, PDO::PARAM_STR);
        $stmt->bindParam(':email', $email, PDO::PARAM_STR);
        $stmt->execute();
    ...
    public function changePassword($token, $newPassword) {
        try {
            ...
            $stmt = $this->pdo->prepare("UPDATE users SET password = :password WHERE verification_token = :token");
            $stmt->bindParam(':password', $hashedPassword, PDO::PARAM_STR);
    ...
    private function getUserDriveSize($userId) {
        ...
        $stmt = $this->pdo->prepare("SELECT drive_size FROM users WHERE id = :userId");
        $stmt->bindParam(':userId', $userId, PDO::PARAM_INT);
        $stmt->execute();
    ...
    private function updateUserDriveSize($userId, $newDriveSize) {
        ...
        $stmt = $this->pdo->prepare("UPDATE users SET drive_size = :newDriveSize WHERE id = :userId");
    ...
    public function processPayment($userId, $amount) {
        ...
        $stmt = $this->pdo->prepare("UPDATE users SET payment_status = 1 WHERE id = :userId");
        $stmt->bindParam(':userId', $userId, PDO::PARAM_INT);
        $stmt->execute();
    }
    ...
    public function upgradeMembershipLevel($userId, $newLevel) {
        ...
        $stmt = $this->pdo->prepare("UPDATE users SET membership_level = :newLevel WHERE id = :userId");
    ...
    public function increaseDriveSize($userId, $additionalSize) {
        ...
        $stmt = $this->pdo->prepare("UPDATE users SET drive_size = drive_size + :additionalSize WHERE id = :userId");
