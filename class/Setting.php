...
class Setting {
    ...
    {
        try {
            $stmt = $this->pdo->prepare("SELECT setting_value FROM settings WHERE setting_name = :settingName");
    ...
    public function updateSetting($settingName, $settingValue)
        {
            try {
                $stmt = $this->pdo->prepare("UPDATE settings SET setting_value = :settingValue WHERE setting_name = :settingName");
